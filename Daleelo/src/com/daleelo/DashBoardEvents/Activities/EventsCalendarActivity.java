...
public class EventsCalendarActivity extends Activity implements OnClickListener
{
	...
	public static String mCalenderBannerImg = "none";
	...
	public void onCreate(Bundle savedInstanceState){	
			...
			heading_txt.setText(""+"Islamic Calendar");
			getParsedData();
			try {
				getBannerData();
		...
		//	_calendarNow  = Calendar.getInstance();
			...
			dayis 	      = _calendar.get(Calendar.DAY_OF_MONTH);
				...
				public void onItemClick(AdapterView<?> arg0, View view,int arg2, long arg3) {					
					...
					if(Sepatator.equalsIgnoreCase("GREY_TRAILING") || Sepatator.equalsIgnoreCase("GREY_EXTRA")){
						...
						if(marrEventDump.size()>0){							
							...
							PopupWindow pWindow = new PopupWindow(EventsCalendarActivity.this);
			...
			mEventList.setOnItemClickListener(new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> arg0, View view,	int position, long arg3) {					
					startActivity(new Intent(EventsCalendarActivity.this,CalendarEventDetailDesc.class)
					.putExtra("data", marrEventDump)
	...
	private void initializeObjects() {
		...
		mEventList 		= new ListView(EventsCalendarActivity.this);
		...
		mCalenderBanner = (ImageView)findViewById(R.id.event_IMG_banner);
	...
	private void getParsedData(){
		...
		EventsCalendarParser mEventsCalenderParser = new EventsCalendarParser(new EventsParserHandler(), url , marrEvent);
	...
	public void onClick(View v){
		...
		if(v.getId()==R.id.today_btn){
	       ...
	       startActivity(new Intent(EventsCalendarActivity.this,EventsDetailsByWeekActivity.class).putExtra("selected_date", sample).putExtra("week", week).putExtra("year", year).putExtra("data", marrEvent).setFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION));  
			...
			startActivity(new Intent(EventsCalendarActivity.this,
			...
			startActivityForResult(new Intent(EventsCalendarActivity.this, EventsCalendarSettingsActivity.class),SETTINGS_RQST_CODE);
			
		}else if(v.getId()==R.id.list_btn){
			
			startActivity(new Intent(EventsCalendarActivity.this,EventsCalendarEventListActivity.class).putExtra("eventslist", marrEvent));
	...
	public class GridCellAdapter extends BaseAdapter 
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			Log.v("shared",""+spinnerPrefs.getInt("position", 3));
			int hijri_correction = spinnerPrefs.getInt("position", 3)-3;
			String islamDate = new EventsIslamicCalendarActivity().getIslamicDate(Double.parseDouble(theday)+hijri_correction, month1 + 1,Double.parseDouble(theyear));
			...
			if (dayis == new Double(theday).intValue()
					&& monthis == new Double(themonth).intValue()
					&& yearis == new Double(theyear).intValue()) {
			...
			if (selection[position]) {
				...
				gridcell1.setTextColor(Color.YELLOW);
			} else {
				row.setBackgroundResource(R.drawable.date);
			}

			if (day_color[1].equals("GREY_TRAILING")
					|| day_color[1].equals("GREY_EXTRA")) {
				gridcell.setTextColor(Color.BLUE);
				gridcell1.setTextColor(Color.RED);
			...
			if (eventsPerMonthMap.containsKey(theday + "-" + themonth + "-"
					+ theyear)) {
				int val = (Integer) eventsPerMonthMap.get(theday + "-"
						+ themonth + "-" + theyear);
	...
	{			
		new AlertDialogMsg(EventsCalendarActivity.this,msg).setPositiveButton("ok", new android.content.DialogInterface.OnClickListener(){
...
//	**************************
	...
	public void getBannerData() throws MalformedURLException{
    	...
    	try {
    		...
    		new mAsyncBannerFetcher(Urls.GET_EVENTS_BANNERS, new BannerParserHandler()).start();
			...
			if(msg.what==0){
				...
				if(mBannerFeeds != null){
					...
					if(mBannerFeeds.get(0).getBannerImageUrl().length()>0){
						...
						mCalenderBannerImg = mBannerFeeds.get(0).getCalanderImage();
						new ImageDownloader().download(String.format(Urls.CALENDER_BANNER_IMG_URL,mCalenderBannerImg), mCalenderBanner);
