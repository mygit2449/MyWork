...
import com.daleelo.TripPlanner.Activities.TripPlannerPointsModel;
import com.daleelo.TripPlanner.Activities.TripPlannerRoadModel;

public class MapDrawPathHelper {
	
	public static TripPlannerRoadModel  getRoute(InputStream is) { 
		KMLHandler handler = new KMLHandler(); 
		try { 
			SAXParser parser = SAXParserFactory.newInstance().newSAXParser(); 
			parser.parse(is, handler); 
		} catch (ParserConfigurationException e) { 
			return handler.mRoad=null;
	...
	public KMLHandler() { 
	mRoad = new TripPlannerRoadModel(); 
	} 
	...
	isPlacemark = true; 
	mRoad.mPoints = addPoints(mRoad.mPoints); 
	...
	private TripPlannerPointsModel[] addPoints(TripPlannerPointsModel[] mPoints) {
		TripPlannerPointsModel[] result = new TripPlannerPointsModel[mPoints.length + 1]; 
		for (int i = 0; i < mPoints.length; i++) 
		result[i] = mPoints[i]; 
		result[mPoints.length] = new TripPlannerPointsModel(); 
		return result; 
	}
