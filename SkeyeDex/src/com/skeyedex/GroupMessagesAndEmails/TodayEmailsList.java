...
public class TodayEmailsList 
{
	private static final String LOG_TAG = TodayEmailsList.class.getSimpleName();
	
	EmailDataBaseHelper mEmailDataBase;
	SQLiteDatabase mDatabase;
	ArrayList<EmailModel> mToday_emails = null;
	Context context;
	Calendar currentDate;
	
	public TodayEmailsList(Context context, Calendar _currentDate){
		
          this.context = context;
          this.currentDate = _currentDate;
	}

	public ArrayList<EmailModel> get_Today_Mails()
	{
	    ...
	    mToday_emails = new ArrayList<EmailModel>();
	    Cursor cursor  = null;
	    int month = currentDate.get(Calendar.MONTH);
	    int day = currentDate.get(Calendar.DATE);
	   
        String st_toDay = currentDate.get(Calendar.YEAR) + "-" + ((month+1>9) ?  (month+1) : "0" + (month+1))  + "-" + ((day>9) ?  (day) : "0" + (day));
        Log.v(LOG_TAG, "Fetching the email for the date " + st_toDay);
        try 
        {
        	  cursor =	mDatabase.rawQuery("select * from ReceivedMails Where EmailDate ='" +st_toDay + "' ORDER BY Email_dateTime DESC", null);
        	 ...
        	 if (cursor.moveToFirst()) 
	         {
	  			 	...
	  			 	{              
                    	  ...
                    	  mToday_emails.add(mCurrentEmail);
